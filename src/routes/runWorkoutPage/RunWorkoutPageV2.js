...
import TranslatedExerciseCompleterV2 from "../../components/exerciseCompleter/TranslatedExerciseCompleterV2";
...
export default function RunWorkoutPageV2() {
  ...
  const finishText = t("finish-text");
  ...
  useEffect(() => {
    ...
    if (Object.keys(lastWorkout).length === 0 || !lastWorkout.exercises) {
      ...
      if (
        !(Object.keys(activeTemplate).length === 0) &&
        ...
        setExerciseCompleters(
          activeTemplate.exercises.map((exercise, index) => {
            ...
            const exerciseName = exerciseIsCommon
              ? processCommonStringFromDb(exercise.name)
              : exercise.name;
            ...
            return (
              <TranslatedExerciseCompleterV2
                ...
                rowsInfo={rows}
                dispatchGenerator={generateDispatchForUpdatingWorkout(
                  exercise.id,
                  index + 1
                )}
                clearExercisesDispatchGenerator={
                  generateClearActiveWorkoutExercisesDispatch
                }
    ...
    // If last workout DOES exist
      ...
      setExerciseCompleters(
        Object.keys(groupedExercises).map((exerciseOrder) => {
          ...
          const exerciseName = exerciseIsCommon
            ? processCommonStringFromDb(setExerciseInfo[0].name)
            : setExerciseInfo[0].name;
          ...
          return (
            <TranslatedExerciseCompleterV2
              ...
              rowsInfo={rows}
              dispatchGenerator={generateDispatchForUpdatingWorkout(
                setExerciseInfo[0].id,
                setExerciseInfo[0].order
              )}
              clearExercisesDispatchGenerator={
                generateClearActiveWorkoutExercisesDispatch
              }
        ...
        <section className="run-workout-page" ref={weightGraphContainerRef}>
          ...
          <button
            ...
            className={`
                            ...
                            ${
                              workoutsLoading
                                ? "run-workout-page__finish-button--disabled"
                                : ""
                            }
                            `}
          ...
          >
            {workoutsLoading ? (
              <div className="spinner-3-rem"></div>
            ) : (
              finishText
            )}
          </button>
