...
export default function ExercisesPage() {
  ...
  const generateAcceptPopupDispatch = (e) => {
    if (callerShowingPopup === buttonNewCaller) {
      ...
      return async (name, description) => {
        dispatch(
          updateExercise({ exerciseId: exerciseIdToEdit, name, description })
        ).then(() => {
          setExerciseIdToEdit("");
          hidePopup(setShowNameDescPopup);
        });
  ...
  const generateDeleteExerciseDispatch = (e) => {
    return () => {
      dispatch(deleteExercise({ exerciseId: exerciseIdToDelete }))
        .then((response) => {
          // Update user's exercises list
          dispatch(
            getExercisesFromUser({
  ...
  return (
    <div
      ...
      onClick={(event) => {
        closePopupOnClickOutside(event, setShowNameDescPopup, [
          "hydrated",
          "button-new",
          "button-new__text",
        ]);
            ...
            {availableExercises.map((exercise) => {
              return (
                <ExercisePresenterV2
                  ...
                  onClickEdit={handleClickShowNameDescPopup(
                    exercisePresenterCaller
                  )(exercise.id)}
