...
export const finishWorkout = createAsyncThunk(
  ...
  async (arg, thunkAPI) => {
    ...
    try {
      ...
      thunkAPI.dispatch(
        slice.actions.restoreActiveWorkout(currentActiveWorkout)
      );
      return false;
    }
  }
...
const slice = createSlice({
  ...
  reducers: {
    updateActiveWorkoutExercise: (state, action) => {
      ...
      const { exerciseId, exerciseOrder, setNumber, weight, reps } =
        action.payload;
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(createWorkout.rejected, (state, action) => {
      ...
      state.error = createNewError(
        action.error.message || "Error creating workout"
      );
    });
    ...
    builder.addCase(deleteWorkout.fulfilled, (state, action) => {
      ...
      state[sliceName].lastNWorkouts = state[sliceName].lastNWorkouts.filter(
      ...
      if (
        state[sliceName].lastWorkout &&
        state[sliceName].lastWorkout.id === workoutId
      ) {
        state[sliceName].lastWorkout = {};
      }
      ...
      if (
        state[sliceName].activeWorkout &&
        state[sliceName].activeWorkout.id === workoutId
      ) {
        state[sliceName].activeWorkout = {};
      }
    ...
    builder.addCase(deleteWorkout.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error deleting workout"
      );
    });
    ...
    builder.addCase(deleteExerciseFromWorkout.fulfilled, (state, action) => {
      ...
      exercises.forEach((exercise) => {
        ...
        if (
          ...
          state[sliceName].activeWorkout.exercises = removeExerciseFromWorkout(
            state[sliceName].activeWorkout.exercises,
            exercise.exerciseId
          );
        }
        ...
        if (
          ...
          state[sliceName].lastWorkout.exercises = removeExerciseFromWorkout(
            state[sliceName].lastWorkout.exercises,
            exercise.exerciseId
          );
        }
        ...
        if (state[sliceName].lastNWorkouts) {
          const workout = state[sliceName].lastNWorkouts.filter(
            (workout) => workout.id === workoutId
          );

          if (workout.length > 0) {
            workout[0].exercises = removeExerciseFromWorkout(
              workout[0].exercises,
              exercise.exerciseId
            );
          }
    ...
    builder.addCase(deleteExerciseFromWorkout.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error deleting exercise from workout"
      );
    });
    ...
    builder.addCase(setLastWorkout.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error setting last workout"
      );
    });
    ...
    builder.addCase(setLastNWorkouts.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error setting last N workouts"
      );
    });
    ...
    builder.addCase(finishWorkout.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error finishing workout"
      );
    });
  },
});
...
export const selectActiveWorkout = (state) =>
  state[sliceName][sliceName].activeWorkout;
...
export const selectLastWorkout = (state) =>
  state[sliceName][sliceName].lastWorkout;
export const selectLastNWorkouts = (state) =>
  state[sliceName][sliceName].lastNWorkouts;
