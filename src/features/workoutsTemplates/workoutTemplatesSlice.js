...
export const updateWorkoutTemplate = createAsyncThunk(
  ...
  async (arg, thunkAPI) => {
    ...
    if (!arg.description || arg.description.trim() === "") {
      ...
      const template = state[sliceName][sliceName].userCreatedTemplates.find(
...
export const getUserRecentWorkouts = createAsyncThunk(
  ...
  async (arg, thunkAPI) => {
    ...
    // map values
    const templates = resolvedPromises.map((template) => {
      return template;
    });
    ...
    const uniqueTemplates = templates.filter((template) => {
      if (collectedIds.has(template.id)) return false;
      collectedIds.add(template.id);
      return true;
    });
    ...
    // Keep only the 5 most recent, unique templates at most
    if (uniqueTemplates.length > 5) {
      uniqueTemplates.splice(5);
    }
...
export const deleteTemplateFromUser = createAsyncThunk(
  ...
  async (arg, thunkAPI) => {
    ...
    const workoutIdsresponse =
      await getWorkoutsIdsAssociatedWithTemplateAndUser({
        templateId,
      });
...
const slice = createSlice({
  ...
  reducers: {
    setActiveTemplate: (state, action) => {
      ...
      const template =
        state[sliceName].userCreatedTemplates.find(
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(createWorkoutTemplate.rejected, (state, action) => {
      ...
      state.error = createNewError(
        action.error.message || "Error creating template"
      );
    });
    ...
    builder.addCase(getAllUserCreatedTemplates.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error getting templates"
      );
    });
    ...
    builder.addCase(getCommonTemplatesForUser.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error getting common templates"
      );
    });
    ...
    builder.addCase(getUserRecentWorkouts.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error getting recent workouts"
      );
    });
    ...
    builder.addCase(updateWorkoutTemplate.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error updating template"
      );
    });
    ...
    builder.addCase(deleteTemplateFromUser.fulfilled, (state, action) => {
      ...
      state[sliceName].userCreatedTemplates = state[
        sliceName
      ].userCreatedTemplates.filter((template) => {
    ...
    builder.addCase(deleteTemplateFromUser.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error deleting template"
      );
    });
    ...
    builder.addCase(deleteExerciseFromTemplate.fulfilled, (state, action) => {
      ...
      if (
        state[sliceName].activeTemplate &&
        state[sliceName].activeTemplate.id &&
        state[sliceName].activeTemplate.id === deletedExercise.workoutTemplateId
      ) {
        state[sliceName].activeTemplate.exercises = state[
          sliceName
        ].activeTemplate.exercises.filter(
          (exercise) => exercise.id !== deletedExercise.exerciseId
        );
      }
    ...
    builder.addCase(deleteExerciseFromTemplate.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error deleting exercise from template"
      );
    });
    ...
    builder.addCase(updateExerciseFromTemplate.fulfilled, (state, action) => {
      ...
      if (
        state[sliceName].activeTemplate &&
        state[sliceName].activeTemplate.id === templateId
      ) {
    ...
    builder.addCase(updateExerciseFromTemplate.rejected, (state, action) => {
      state.isLoading.pop();
      state.error = createNewError(
        action.error.message || "Error updating exercise from template"
      );
    });
  },
});
...
// Export selectors
export const selectUserTemplates = (state) =>
  state[sliceName][sliceName].userCreatedTemplates;
export const selectCommonTemplates = (state) =>
  state[sliceName][sliceName].commonTemplates;
export const selectActiveTemplate = (state) =>
  state[sliceName][sliceName].activeTemplate;
...
export const selectRecentWorkouts = (state) =>
  state[sliceName][sliceName].recentWorkouts;
