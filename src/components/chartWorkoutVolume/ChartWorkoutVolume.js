...
const ChartWorkoutVolume = ({
  ...
  const processData = (data) => {
    ...
    const uniqueData = Array.from(
      withComparison
        .reduce((map, item) => map.set(item.x, item), new Map())
        .values()
    );
  ...
  if (isLoading) {
    return (
      <div className="spinner-10-rem chart-workout-volume__chart-spinner"></div>
    );
  }
      ...
      tooltip={({ point }) => {
        ...
        return (
          <div className="tooltip">
            ...
            <p>
              {volumeText}:{" "}
              <span className="tooltip__data">{point.data.y}</span>
            </p>
          </div>
        );
      }}
