...
const ChartSetsAndWeight = ({
  ...
  const processData = (data) => {
    const processedData = data.flatMap((dataPoint, dayIndex) => {
      ...
      dataPoint.sets.forEach((set, setIndex) => {
        processedPoint = {
          ...processedPoint,
          setIndex, // TODO later? REVISAR Para alinear con el punto
          [`set${set.set}`]: set.set,
    ...
    const uniqueData = Array.from(
      processedData
        .reduce((map, item) => map.set(item.datetime, item), new Map())
        .values()
    );
  ...
  const convertWeightToRepsScale = (weight) => {
    return (
      ((maxReps - 1) * (weight - weightRange[0])) /
      (weightRange[1] - weightRange[0])
    );
  };
  ...
  const convertRepsToWeightScale = (reps) => {
    return (
      weightRange[0] +
      ((weightRange[1] - weightRange[0]) * reps) / (maxReps - 1)
    );
  };
  ...
  useEffect(() => {
    ...
    let weightRangeFinder = newData.reduce(
      (weightRange, dataPoint) => {
    ...
    if (weightRangeFinder[0] === weightRangeFinder[1]) {
      ...
      weightRangeFinder = [
        Math.max(0, weightRangeFinder[1] - 1),
        weightRangeFinder[1],
      ];
    }
  ...
  const chartMargin = isSmall
    ...
    : { top: 50, right: 80, bottom: 80, left: 60 };
  ...
  return (
    <ResponsiveBar
      ...
      keys={Array.from(
        new Set(
          barData.flatMap((dataPoint) =>
            Object.keys(dataPoint).filter((key) => key.startsWith("reps"))
          )
        )
      )}
      groupMode="grouped"
          ...
          return (
            !isSmall && (
              <g transform={`translate(${tick.x},${tick.y})`}>
                <line x1={0} x2={-6} y1={0} y2={0} stroke={colorWeight} />
                <text
                  x={6}
                  y={4}
                  textAnchor="start"
      ...
      tooltip={({ id, value, data }) => {
        ...
        return (
          <div className="tooltip">
            ...
            <p>
              {repsText}: <span className="tooltip__data">{value}</span>
            </p>
            <p>
              {weightText}:{" "}
              <span className="tooltip__data tooltip__data--red">{weight}</span>
            </p>
          </div>
        );
      }}
      layers={["grid", "axes", "bars", LineLayer, "legends", "markers"]}
